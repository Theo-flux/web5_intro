...
var DwnDataStore = class {
  constructor() {
     ...
     */
    this._protocolInitializedCache = new import_common3.TtlCache({ ttl: (0, import_ms.default)("1 hour"), max: 1e3 });
    /**
...
var AgentDwnApi = class {
  ...
  async getDwnMessage({ author, messageCid }) {
    ...
    if (isRecordsWrite(messageEntry) && messageEntry.data) {
      const dataBytes = await import_common6.NodeStream.consumeToBytes({ readable: messageEntry.data });
...
var DwnRegistrar = class _DwnRegistrar {
  ...
  static async hashAsHexString(input) {
    const hashAsBytes = await import_crypto11.Sha256.digest({ data: import_common7.Convert.string(input).toUint8Array() });
    const hashAsHex = import_common7.Convert.uint8Array(hashAsBytes).toHex();
    return hashAsHex;
  }
  /**
  ...
  static async generateNonce() {
    const randomBytes2 = import_crypto11.utils.randomBytes(32);
    const hexString = await import_common7.Convert.uint8Array(randomBytes2).toHex().toUpperCase();
    return hexString;
  }
};
...
var DwnKeyStore = class extends DwnDataStore {
  ...
  async getAllRecords({ agent, tenantDid }) {
    ...
    for (const record of queryReply.entries ?? []) {
      ...
      const storedKey = import_common8.Convert.base64Url(record.encodedData).toObject();
  ...
  static async decrypt({
    ...
    switch (joseHeader.alg) {
      ...
      case "PBES2-HS512+A256KW": {
        ...
        try {
          salt = new Uint8Array([
            ...import_common9.Convert.string(joseHeader.alg).toUint8Array(),
            0,
            ...import_common9.Convert.base64Url(joseHeader.p2s).toUint8Array()
          ]);
        } catch {
...
function decodeHeaderParam(param, value) {
  ...
  try {
    if (typeof value !== "string")
      throw new Error();
    return import_common10.Convert.base64Url(value).toUint8Array();
  } catch {
    throw new CryptoError(
...
var FlattenedJwe = class _FlattenedJwe {
  ...
  static async decrypt({
    ...
    if (jwe.protected) {
      try {
        parsedProtectedHeader = import_common10.Convert.base64Url(jwe.protected).toObject();
      } catch {
    ...
    let cek;
    try {
      const encryptedKey = jwe.encrypted_key ? import_common10.Convert.base64Url(jwe.encrypted_key).toUint8Array() : void 0;
    ...
    const ciphertext = tag !== void 0 ? new Uint8Array([
      ...import_common10.Convert.base64Url(jwe.ciphertext).toUint8Array(),
      ...tag ?? []
    ]) : import_common10.Convert.base64Url(jwe.ciphertext).toUint8Array();
    const additionalData = jwe.aad !== void 0 ? new Uint8Array([
      ...import_common10.Convert.string(jwe.protected ?? "").toUint8Array(),
      ...import_common10.Convert.string(".").toUint8Array(),
      ...import_common10.Convert.string(jwe.aad).toUint8Array()
    ]) : import_common10.Convert.string(jwe.protected ?? "").toUint8Array();
  ...
  static async encrypt({
    ...
    switch (joseHeader.enc) {
      ...
      case "A192GCM":
      case "A256GCM":
        iv = import_crypto15.utils.randomBytes(12);
        break;
      default:
        iv = new Uint8Array(0);
    }
    const encodedProtectedHeader = protectedHeader ? import_common10.Convert.object(protectedHeader).toBase64Url() : "";
    let additionalData;
    let encodedAad;
    if (additionalAuthenticatedData) {
      encodedAad = import_common10.Convert.uint8Array(additionalAuthenticatedData).toBase64Url();
      additionalData = import_common10.Convert.string(encodedProtectedHeader + "." + encodedAad).toUint8Array();
    } else {
      additionalData = import_common10.Convert.string(encodedProtectedHeader).toUint8Array();
    }
    ...
    const jwe = new _FlattenedJwe({
      ciphertext: import_common10.Convert.uint8Array(ciphertext).toBase64Url()
    });
    if (encryptedKey)
      jwe.encrypted_key = import_common10.Convert.uint8Array(encryptedKey).toBase64Url();
    if (protectedHeader)
    ...
    if (iv)
      jwe.iv = import_common10.Convert.uint8Array(iv).toBase64Url();
    if (encodedAad)
      jwe.aad = encodedAad;
    if (authenticationTag)
      jwe.tag = import_common10.Convert.uint8Array(authenticationTag).toBase64Url();
    return jwe;
  }
};
...
var HdIdentityVault = class {
  ...
  constructor({ keyDerivationWorkFactor, store } = {}) {
    ...
    this._store = store ?? new import_common11.MemoryStore();
  }
  /**
  ...
  async backup() {
    ...
    const backupDataString = import_common11.Convert.object(backupData).toBase64Url();
    const backup = {
  ...
  async changePassword({ oldPassword, newPassword }) {
    ...
    try {
      ...
      ({ plaintext: contentEncryptionKeyBytes, protectedHeader } = await CompactJwe.decrypt({
        jwe: cekJwe,
        key: import_common11.Convert.string(oldPassword).toUint8Array(),
        crypto: this.crypto,
        keyManager: new LocalKeyManager2()
      }));
      contentEncryptionKey = import_common11.Convert.uint8Array(contentEncryptionKeyBytes).toObject();
    } catch (error) {
    ...
    const newCekJwe = await CompactJwe.encrypt({
      key: import_common11.Convert.string(newPassword).toUint8Array(),
      protectedHeader,
      // Re-use the protected header from the original JWE.
      plaintext: import_common11.Convert.object(contentEncryptionKey).toUint8Array(),
      crypto: this.crypto,
      keyManager: new LocalKeyManager2()
    });
  ...
  async getDid() {
    ...
    const portableDid = import_common11.Convert.uint8Array(portableDidBytes).toObject();
  ...
  async getStatus() {
    ...
    const vaultStatus = import_common11.Convert.string(storedStatus).toObject();
  ...
  async initialize({ password, recoveryPhrase }) {
    ...
    const cekJweProtectedHeader = {
      ...
      p2s: import_common11.Convert.uint8Array(saltInput).toBase64Url()
    };
    const cekJwe = await CompactJwe.encrypt({
      key: import_common11.Convert.string(password).toUint8Array(),
    ...
    const didJwe = await CompactJwe.encrypt({
      ...
      plaintext: import_common11.Convert.object(portableDid).toUint8Array(),
  ...
  async restore({ backup, password }) {
    ...
    try {
      const backupData = import_common11.Convert.base64Url(backup.data).toObject();
  ...
  async unlock({ password }) {
    ...
    try {
      ...
      const contentEncryptionKey = import_common11.Convert.uint8Array(contentEncryptionKeyBytes).toObject();
      this._contentEncryptionKey = contentEncryptionKey;
    } catch (error) {
...
var DwnIdentityStore = class extends DwnDataStore {
  ...
  async getAllRecords({ agent, tenantDid }) {
    ...
    for (const record of queryReply.entries ?? []) {
      ...
      const storedIdentity = import_common12.Convert.base64Url(record.encodedData).toObject();
...
var DwnServerInfoCacheMemory = class {
  constructor({ ttl = "15m" } = {}) {
    this.cache = new import_common13.TtlCache({ ttl: (0, import_ms3.default)(ttl) });
  }
  /**
...
var JsonRpcSocket = class _JsonRpcSocket {
  ...
  async request(request) {
    return new Promise((resolve, reject) => {
      request.id ??= import_crypto18.utils.randomUuid();
...
var WebSocketDwnRpcClient = class _WebSocketDwnRpcClient {
  ...
  static async subscriptionRequest(connection, target, message, messageHandler) {
    const requestId = import_crypto19.utils.randomUuid();
    const subscriptionId = import_crypto19.utils.randomUuid();
...
var HttpWeb5RpcClient = class extends HttpDwnRpcClient {
  async sendDidRequest(request) {
    const requestId = import_crypto20.utils.randomUuid();
...
var SyncEngineLevel = class _SyncEngineLevel {
  ...
  async pull() {
    ...
    for (let job of pullJobs) {
      ...
      const dataStream = isRecordsWrite(replyEntry) && replyEntry.data ? import_common14.NodeStream.fromWebReadable({ readableStream: replyEntry.data }) : void 0;
  ...
  static syncMessageReplyIsSuccessful(reply) {
    return reply.status.code === 202 || reply.status.code === 409 || // If the message is a RecordsDelete and the status code is 404, the initial write message was not found or the message was already deleted
    reply.entry?.message.descriptor.interface === import_dwn_sdk_js5.DwnInterfaceName.Records && reply.entry?.message.descriptor.method === import_dwn_sdk_js5.DwnMethodName.Delete && reply.status.code === 404;
  }
  ...
  async getDwnEventLog({ did, dwnUrl, syncDirection, cursor }) {
    ...
    if (syncDirection === "pull") {
      ...
      messagesReply = messagesReadDwnResponse.reply;
    }
  ...
  async getDwnMessage({ author, messageCid }) {
    ...
    if (isRecordsWrite(messageEntry) && messageEntry.data) {
      const dataBytes = await import_common14.NodeStream.consumeToBytes({ readable: messageEntry.data });
      dwnMessageWithBlob.data = new Blob([dataBytes], { type: messageEntry.message.descriptor.dataFormat });
    }
    return dwnMessageWithBlob;
  }
...
var PlatformAgentTestHarness = class _PlatformAgentTestHarness {
  ...
  static async setup({ agentClass, agentStores, testDataLocation }) {
    ...
    const dwnResumableTaskStore = new import_dwn_sdk_js6.ResumableTaskStoreLevel({ location: testDataPath("DWN_RESUMABLETASKSTORE") });
  ...
  static useDiskStores({ agent, testDataLocation, stores }) {
    const testDataPath = (path) => `${testDataLocation}/${path}`;
    const vaultStore = new import_common15.LevelStore({ location: testDataPath("VAULT_STORE") });
  ...
  static useMemoryStores({ agent } = {}) {
    const vaultStore = new import_common15.MemoryStore();
    const agentVault = new HdIdentityVault({ keyDerivationWorkFactor: 1, store: vaultStore });
